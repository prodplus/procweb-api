...
public class NotImpugnacao {
	...
	public static ByteArrayInputStream gerar(Processo processo, Fornecedor fornecedor) {
		...
		try {
			...
			conteudo = new Paragraph("1) autoridade julgadora a quem é dirigido;");
			conteudo.setAlignment(Element.ALIGN_LEFT);
			document.add(conteudo);
			conteudo = new Paragraph("2) qualificação do impugnante;");
			conteudo.setAlignment(Element.ALIGN_LEFT);
			document.add(conteudo);
			conteudo = new Paragraph(
					"3) as razões de fato e de direito que fundamentam a impugnação;");
			conteudo.setAlignment(Element.ALIGN_LEFT);
			document.add(conteudo);
			conteudo = new Paragraph("4) as provas que lhe dão suporte;");
			conteudo.setAlignment(Element.ALIGN_LEFT);
			document.add(conteudo);
			...
			String paragrafo2 = "Cumpre informá-lo que o processo administrativo tem por base "
					...
					+ "constam da cópia do inteiro teor da reclamação que a esta acompanha.";
